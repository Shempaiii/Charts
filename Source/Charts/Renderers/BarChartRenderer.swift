            ...
            for dataSetIndex in 0 ..< barData.dataSetCount
            {
                ...
                else
                {
                    ...
                    for index in 0 ..< Int(ceil(Double(dataSet.entryCount) * animator.phaseX))
                    {
                        ...
                        let x = rect.origin.x + rect.size.width / 2.0
                        var y = rect.origin.y + rect.size.height / 2.0
                        ...
                        else
                        {
                            ...
                            var transformedHeight: [CGRect] = []
                            switch index {
                            case 0:
                                transformedHeight = Array(buffer.rects[0...1])
                                break
                            case 1:
                                transformedHeight = Array(buffer.rects[2...3])
                                break
                            case 2:
                                transformedHeight = Array(buffer.rects[4...5])
                                break
                            default:
                                break
                            }
                            
                            var currentOffset: CGFloat = 1.5
                            for k in 0 ..< transformed.count
                            {
                                let val = vals[k]
                                var y : CGFloat = y
                                if k == 0 {
                                    y = y - valueTextHeight / 2.0
                                }
                                if k == 1 {
                                    y = transformedHeight[0].origin.y - (transformedHeight[k].height / 2) - (valueTextHeight / 2.0)
                                }
                                currentOffset += rect.size.height// transformed[k].y
                                if !viewPortHandler.isInBoundsRight(x)
                                {
                                    break
                                }
